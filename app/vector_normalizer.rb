...
module VectorNormalizer
  ...
  # Sakamoto DOW lookup table: avoids Time object allocation when last_tx is nil
  ...
  def self.call(payload)
    ...
    Numo::SFloat[
      clamp(amount / NORM['max_amount']),
      clamp(tx['installments'].to_f / NORM['max_installments']),
      avg_amount.zero? ? 0.0 : clamp((amount / avg_amount) / NORM['amount_vs_avg_ratio']),
      hour.to_f / 23.0,
      day.to_f / 6.0,
      dim5,
      dim6,
      clamp(terminal['km_from_home'].to_f / NORM['max_km']),
      clamp(customer['tx_count_24h'].to_f / NORM['max_tx_count_24h']),
      ...
      clamp(merchant['avg_amount'].to_f / NORM['max_merchant_avg_amount'])
    ]
  end
  ...
  def self.last_tx_dimensions(last_tx, requested_at_str)
    ...
    [
      clamp(minutes_diff / NORM['max_minutes']),
      clamp(last_tx['km_from_current'].to_f / NORM['max_km'])
    ]
  end
